...
const MessageView: React.FC = () => {
    ...
    // Setup WebSocket connection and fetch initial messages
    useEffect(() => {
        if (!userData?.id || !selectedUser) return;
        ...
        client.connect({}, () => {
            ...
            // Fetch initial messages only once when WebSocket connects
            getMessages();
        });
    ...
    }, [stompClient, selectedUser, userData]);
    ...
    }, [stompClient, message, selectedFile, selectedUser, userData, sendTypingIndicator]);
    ...
    const handleFileSave = useCallback(async (): Promise<void> => {
        if (!selectedFile || !selectedUser || !token) return;
    ...
    }, [selectedFile, selectedUser, token, userData]);
    ...
    const formatTime = useCallback((dateString: Date | string) => {
        const date = typeof dateString === 'string' ? new Date(dateString) : dateString;
        return date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    }, []);
    ...
    const MessageSkeleton = useCallback(() => {
        return (
            <div className="space-y-6 p-4">
                {/* Date divider skeleton */}
                <div className="flex justify-center">
